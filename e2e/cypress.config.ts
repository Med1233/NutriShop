    ...
    async setupNodeEvents(on, config) {
      ...
      on('task', {
        async 'db:reset'() {
          ...
          const pool = new Pool({ connectionString: 'postgresql://admin:password@localhost:5432/appdb' });
          await pool.query('DELETE FROM order_items');
          await pool.query('DELETE FROM cart_items');
          await pool.query('DELETE FROM orders');
          await pool.query('DELETE FROM refresh_tokens');
          await pool.query("DELETE FROM users WHERE email != 'macinessa365@gmail.com'");
          await pool.end();
          return null;
        },
